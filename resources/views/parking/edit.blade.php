...
@section('content')
    <div class="container">
        <h1 class="text-center mb-4">Промяна на разходи за паркинг на {{$truck}}</h1>
        <div class="align-content-center">
            <div>
                <form method="post" action="{{route('parking.update', $parking->id)}}">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control col-md-6 mb-2 float-left" name="month" id="month">
                            @for($i = 1; $i<= 12; $i++)
                                <option
                                    value="{{$i}}" {{$parking->reportingPeriod->month == $i ? 'selected' : ''}}>{{date("F", mktime(0, 0, 0, $i, 10))}}</option>
                            @endfor
                        </select>
                        <select class="form-control col-md-6" name="year" id="year">
                            @for($i = -1; $i <= 1; $i++)
                                <option value="{{date("Y")+$i}}"
                                    {{(date("Y")+$i == date($parking->reportingPeriod->year)) ? 'selected' : ''}}>
                                    {{date("Y")+$i}}</option>
                            @endfor
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="number" name="price" min="0" step="0.01"
                               placeholder="Цена : {{$parking->price}} "
                               class="form-control float-right col-md-6  @error('price') is-invalid @enderror"
                               id="price" value="{{old('price') ?  old('price') : $parking->price}}">
                        @error('price')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" step="0.01" min="0.01" max="9.99"
                               placeholder="Старо показание : {{$parking->note}}"
                               name="note"
                               class="form-control float-right col-md-6  @error('note') is-invalid @enderror"
                               id="note"
                               value="{{old('note') ? old('note') : $parking->note}}">
                        @error('note')
                        <div class="col-md-12 float-right text-right text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <form action="{{ route('parking.destroy', ['parking_id'=> $parking->id]) }}" method="POST">
                    @method('delete')
                    @csrf
